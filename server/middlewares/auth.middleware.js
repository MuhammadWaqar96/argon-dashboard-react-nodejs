...
module.exports.authentication = async (req, res, next) => {
    ...
    if (!token && req.headers.authentication && req.headers.authentication.startsWith('Bearer ') && req.originalUrl !="/api/auth/refresh") {
    ...
    try {
        ...
        if (error instanceof jwt.TokenExpiredError) {
            return res.status(401).json({ ok: false, error: 'jwt.expired_token' })
        }
    }
